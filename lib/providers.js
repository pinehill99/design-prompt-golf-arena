/* Real BYOK provider adapters. Calls go browser -> provider directly or through
   a user-provided proxy. Keys/session handles live only in localStorage on this device.
...
(function () {
  ...
  function proxyUrl(base, path) {
    return cleanBaseUrl(base) + "/" + String(path || "").replace(/^\/+/, "");
  }
  ...
  /* ---------------- OpenAI Codex login via proxy ----------------
     Codex login is a CLI/trusted-environment flow. This static app does not
     handle Codex account tokens; it calls a user-controlled proxy whose login
     endpoint stores Codex auth server-side and exposes an OpenAI-compatible
     chat endpoint for the browser. */
  const codex = {
    id: "codex", label: "Codex Login", keyHint: "proxy session token", docs: "developers.openai.com/codex",
    proxyHint: "https://your-worker.example.com",
    models: openai.models,
    visionModel: openai.visionModel,
    supportsEffort: true,
    isConfigured: (keys) => !!(cleanBaseUrl(keys.codexProxy) && (keys.codexSession || keys.codexUseCookie)),
    loginUrl(keys) {
      const base = cleanBaseUrl(keys.codexProxy);
      return base ? proxyUrl(base, "/auth/codex/start") + "?return_to=" + encodeURIComponent(location.href) : "";
    },
    async _call(key, keys, body, signal) {
      const base = cleanBaseUrl(keys.codexProxy);
      const session = String(keys.codexSession || "").trim();
      const useCookie = !!keys.codexUseCookie;
      if (!base) throw new Error("Add a Codex login proxy URL in Settings.");
      if (!session && !useCookie) throw new Error("Open Codex login or add a proxy session in Settings.");
      const headers = {
        "content-type": "application/json",
        "x-prompt-golf-openai-endpoint": "/v1/chat/completions",
      };
      if (session) headers.authorization = "Bearer " + session;
      const res = await fetch(proxyUrl(base, "/openai-chat"), {
        method: "POST", signal,
